...
As I mentioned above, the data was collected from an experimental design of 154
individuals that were spread across 5 blocks. In each block the individuals were
infected with 3 pathogens.

In this section I will show you the position of the individuals in the plots of
the experimental design (position is given by X and Y coordinates). Most
importantly, the plots will give you the idea the variability in individuals' 
responses in terms of latent period (LP), uredinia number (UN), and uredinia
size (US) across the pathogens.
...
From these plots, I can see that poplar individuals tended to start showing
infection symptoms as early as day 8 (incubation period = 14 days). This is true
for the poplars infected by pathogen 93JE3 and P72. The poplars that were
infected by 09AX27 showed more variable response.
...
UN_variation
```
It is not very easy to compare UN variation across pathogens. There is a high 
range for UN for each pathogen. However, I can see that individuals infected 
by pathogen 09AX27 showed smaller amount of uredinia than the other individuals
infected by 93JE3 and P72. Individuals infected by P72 showed the largest amount
of uredinia, but most of them were showing uredinia number far below the largest
amount. 
...
# The overall response of individuals toward the infection.
As I mentioned above, I had an experimental design of poplar individuals spread 
across 5 blocks. So, now I want to see the overall response of the individuals
regardless of the blocks. To do this, I will need to average individual response
by dividing it with 5, which is the number of blocks we have.

## For variable latent period
```{r LP mean sd}
...
```{r LP mean ggplot, message=FALSE, warning=FALSE}
...
```{r UN mean ggplot 09AX27, message=FALSE, warning=FALSE}
