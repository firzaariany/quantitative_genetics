...
The general aim of the following study was to find the genetic potential
of black poplar's resistance (*Populus nigra*) when they are infected by
rust.
...
Exploratory Data Analysis (EDA) to gather insights from the dataset.
...
discriminants) spread across 5 blocks were inoculated with 3 rust
strains. These make up a total 2520 observations of 15 variables.
...
3.  Understand the variability in distribution of the phenotypes for
    each strain.
...
rust id on the top shows the type of pathogen that infected the poplar.
This graph shows the share of resistant and susceptible poplars when
they were infected with different pathogen types. The poplars were
behaving quite similarly. There were around 700 susceptible poplars.
These are the poplars that I studied to see whether they were behaving
differently or not. Also, whether they had the chance to fight the
pathogen.
...
So I see that latent period (LP) and uredinia number (UN) are skewed to
the left, while uredinia size (US) was discrete. I will know what
happens to these in\
future analysis.

Now I want to cross check the previous distribution (that was not
...
pattern in the interaction between poplars and their pathogen.
